...
class DeckRepositoryImpl(val deckDao: DeckDao) : DeckRepository {
    ...
    override fun delete(deck: Deck) =
        deckDao.delete(deck)

    override fun getAllDecks(): LiveData<List<Deck>> =
        deckDao.getAllDecks()

    override fun getDeckById(deck_id: Long?) : Deck? =
        deckDao.getDeckById(deck_id)

    override fun getDeckWithCards(deck_id: Long): DeckWithCards =
        deckDao.getDeckWithCards(deck_id)
}